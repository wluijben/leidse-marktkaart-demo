...
// Helper metadata for stalls and landmarks
...
export function parseMarketMarkers(dataset: MarketDataset = rawMarketData): MarkerItem[] {
  return dataset.markers.map((m, index) => {
    const textUpper = m.text.trim().toUpperCase();
    const isNumber = /^\d+$/.test(textUpper);
    ...
    if (isNumber) {
      ...
      return {
        ...
        type: 'stall',
        zone,
        category: stallInfo.category,
        description: stallInfo.description,
        typicalProducts: stallInfo.products
      };
    } else {
      ...
      return {
        ...
        typicalProducts: landmark.typicalProducts
      };
    }
  });
}
